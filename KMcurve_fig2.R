...
# 2) Whole cohort 시트에서 플롯 생성 함수
plot_whole <- function() {
  
  df <- read_excel(paste0(file_path,"/CVD_mortality_results_v5.xlsx"), sheet = "whole") %>% # whole male female
    mutate(
  ...
  # Family history + mCVH score 기반 그룹 라벨 생성
  df <- df %>%
    mutate(
      FH = ifelse(grepl("\\.1$", variable), "Yes FH", "No FH"),
      Lifestyle = ifelse(grepl("^High", variable),
                         "High mCVH score", "Low mCVH score"),
...
# ggsave("Desktop/통계대학원/프로젝트/강북삼성/paper/CVD/analysis_results/men_figure.png",
#        plot = whole_plot,
#        width = 10, height = 6, dpi = 600)
# 
# ggsave("Desktop/통계대학원/프로젝트/강북삼성/paper/CVD/analysis_results/women_figure.png",
#        plot = whole_plot,
#        width = 10, height = 6, dpi = 600)
